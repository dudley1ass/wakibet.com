...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tournament test-run files from a player data workbook.")
    parser.add_argument("--source", required=True, help="Path to source xlsx file.")
    parser.add_argument(
        "--name",
        required=True,
        help="Tournament display name (used in summary).",
    )
    parser.add_argument(
        "--sheet",
        default="Registrations",
        help='Worksheet name (default: "Registrations").',
    )
    parser.add_argument(
        "--out-dir",
    ...
    tournament_slug = file_slug(args.name)

    wb = load_workbook(source_path, data_only=True)
    selected_sheet = args.sheet
    if selected_sheet not in wb.sheetnames:
        registrations_sheet = next(
            (name for name in wb.sheetnames if "registration" in name.lower()),
            None,
        )
        if registrations_sheet:
            selected_sheet = registrations_sheet
        else:
            raise ValueError(
                f'Sheet "{args.sheet}" not found. Available: {", ".join(wb.sheetnames)}'
            )
    ...
    summary = {
        "tournament_name": f"{args.name} (test run)",
    ...
    json_path = out_dir / f"{tournament_slug}_test_run_matches.json"
    csv_path = out_dir / f"{tournament_slug}_player_match_list.csv"
    summary_path = out_dir / f"{tournament_slug}_test_run_summary.json"
